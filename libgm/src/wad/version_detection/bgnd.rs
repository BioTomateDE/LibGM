...
pub fn check_2024_14_1(reader: &mut DataReader) -> Result<Option<GMVersion>> {
    ...
    for i in 0..count {
        // Find background's start position, and calculate next background position (if
        // available).
        ...
        // Skip all the way to "GMS2ItemsPerTileCount" (at its pre-2024.14.1 location),
        // which is what we actually care about.
        reader.cur_pos = bg_ptr + 44;
        let items_per_tile_count = reader.read_u32()?;
        let tile_count = reader.read_u32()?;

        // Calculate the theoretical end position given the above info, and compare to
        // the actual end position (with padding).
