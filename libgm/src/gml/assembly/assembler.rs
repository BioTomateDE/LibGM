...
pub fn assemble_instruction(
    ...
    let opcode_end: Option<usize> = reader.line.find(|c| c == ' ' || c == '.');
    ...
    while reader.starts_with(".") {
        reader.consume_dot()?;
        let raw_type: char = reader.consume_char().ok_or(
            "Unexpected EOL when trying to parse instruction data type",
        )?;
    ...
    let instruction: GMInstruction = match mnemonic.as_str() {
        ...
        "add" => {
            types.assert_count(2, &mnemonic)?;
            GMInstruction::Add { augend: types[0], addend: types[1] }
        }
        "sub" => {
            types.assert_count(2, &mnemonic)?;
        ...
        "shr" => {
            types.assert_count(2, &mnemonic)?;
            GMInstruction::ShiftRight {
                value: types[1],
                shift_amount: types[0],
            }
        }
        "cmp" => parse_comparison(types, &mut reader)?,
        "pop" => {
            types.assert_count(2, &mnemonic)?;
            let variable: CodeVariable = parse_variable(&mut reader, &gm_data)?;
            GMInstruction::Pop { type1: types[0], type2: types[1], variable }
        }
        "popswap" => {
        ...
        "dup" => parse_duplicate(types, &mut reader)?,
        "dupswap" => parse_duplicate_swap(types, &mut reader)?,
        "ret" => {
            types.assert_count(0, &mnemonic)?;
        ...
        "push" => GMInstruction::Push {
            value: parse_push(types, &mut reader, gm_data)?,
        },
        "pushloc" => {
            types.assert_count(0, &mnemonic)?;
            let variable = parse_variable(&mut reader, gm_data)?;
        ...
        "call" => parse_call(types, &mut reader, gm_data)?,
        "callvar" => {
            types.assert_count(0, &mnemonic)?;
        ...
        "pushref" => {
            types.assert_count(0, &mnemonic)?;
            let asset_reference = parse_asset_reference(&mut reader, gm_data)?;
...
fn parse_asset_reference(
    reader: &mut Reader,
    gm_data: &GMData,
) -> Result<GMAssetReference> {
    let line = reader.line;
    let asset_type = reader.consume_round_brackets()?.ok_or_else(|| {
        format!("Expected asset type within round brackets; found {line:?}")
    })?;
...
fn parse_push(
    ...
    let value: GMCodeValue = match types[0] {
        GMDataType::Int16 => GMCodeValue::Int16(reader.parse_int()?),
        GMDataType::Int32 => {
            if let Some(type_cast) = reader.consume_round_brackets()? {
                match type_cast {
                    "function" => GMCodeValue::Function(parse_function(
                        reader,
                        &gm_data.functions,
                    )?),
                    "variable" => {
                        let mut variable: CodeVariable =
                            parse_variable(reader, gm_data)?;
                ...
                GMCodeValue::Int32(reader.parse_int()?)
            }
        }
        GMDataType::Int64 => GMCodeValue::Int64(reader.parse_int()?),
        GMDataType::Double => {
            let float: &str = reader.clear();
            let float: f64 = float.parse().context("parsing float literal")?;
            GMCodeValue::Double(float)
        }
        GMDataType::Boolean => {
            let bool: &str = reader.parse_identifier()?;
        ...
        GMDataType::Variable => {
            GMCodeValue::Variable(parse_variable(reader, gm_data)?)
        }
    };
    Ok(value)
}
...
fn parse_call(
    ...
    let function: GMRef<GMFunction> =
        parse_function(reader, &gm_data.functions)?;
...
impl GMVariableType {
    fn from_string(variable_type: &str) -> Result<GMVariableType> {
        Ok(match variable_type {
            "stacktop" => GMVariableType::StackTop,
            "array" => GMVariableType::Array,
            "roominstance" => GMVariableType::Instance,
...
fn parse_variable(
    ...
    let instance_type: GMInstanceType = match instance_type_raw.as_str() {
        ...
        "local" => {
            let var_index: u32 = instance_type_arg.parse().with_context(|| {
                format!("Invalid index for local variable: {instance_type_arg:?}")
            })?;
        ...
        "roominstance" => {
            ...
            let instance_id: i16 =
                instance_type_arg.parse().with_context(|| {
                    format!("Invalid Room Instance ID: {instance_type_arg:?}")
                })?;
...
fn parse_function_identifier(reader: &mut Reader) -> Result<String> {
    ...
    if reader.consume_str("@@").is_some() {
        if let Ok(ident) = reader.parse_identifier() {
