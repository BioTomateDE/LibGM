...
fn list_dir(dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    let dir: ReadDir = fs::read_dir(dir)
        ...
        .ctx("reading data file folder")?;

    for entry in dir {
        let entry = entry
            .map_err(|e| e.to_string())
            .ctx("reading directory entry metadata")?;
...
pub fn get_data_files(input_files: &[PathBuf]) -> Result<Vec<PathBuf>> {
    ...
    for path in input_files {
        let metadata = fs::metadata(path)
            .map_err(|e| e.to_string())
            .ctx(|| format!("reading metadata of {path:?}"))?;

        if metadata.is_dir() {
            let dir_files: Vec<PathBuf> =
                list_dir(path).ctx(|| format!("reading entries of directory {path:?}"))?;
