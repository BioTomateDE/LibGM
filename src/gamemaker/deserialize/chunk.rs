...
impl DataReader<'_> {
    ...
    pub fn read_chunk_required<T: GMChunkElement + GMElement>(&mut self, chunk_name: &str) -> Result<T> {
        let chunk: GMChunk = self.chunks.get(chunk_name).cloned().ok_or_else(|| format!(
    ...
    fn unstable_get_gm_version(&mut self) -> Result<GMVersion> {
        ...
        self.chunk = self.chunks.get("GEN8").cloned().context("Chunk GEN8 does not exist").context(CTX)?;
