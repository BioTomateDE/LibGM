...
impl GMElement for GMTextureGroupInfo {
    fn deserialize(reader: &mut DataReader) -> Result<Self, String> {
        ...
        let spine_sprites_ptr: usize = if !reader.general_info.is_version_at_least((2023, 1, Post2022_0)) {
            reader.read_usize()?
        } else { 0 };
        ...
        let spine_sprites: Vec<GMRef<GMSprite>> = if !reader.general_info.is_version_at_least((2023, 1, Post2022_0)) {
    ...
    fn serialize(&self, builder: &mut DataBuilder) -> Result<(), String> {
        ...
        if !builder.is_gm_version_at_least((2023, 1, Post2022_0)) {
