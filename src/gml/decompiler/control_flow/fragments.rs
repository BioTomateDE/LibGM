...
#[derive(Debug, Clone)] // TODO: mutable blocks but more cancer
    ...
    for i in 0..cfg.blocks.len() {
        ...
        if block.start_address == current.end_address {
            if stack.is_empty() {
                ...
                current.children.push(NodeIndex::Block(i));
                current.blocks.push(NodeIndex::Block(i));
            ...
            if let Some(NodeIndex::Block(idx)) = current.blocks.last() {
                let last_block = &mut cfg.blocks[*idx];
                match last_block.instructions.last() {
                    ...
                    None => unreachable!("Block doesn't have any instructions"),
                }
            } else {
        ...
        if let Some(child_code) = child_start_offsets.get(&cfg.blocks[i].start_address) {
            ...
            let Successors::UnconditionalBranch(branch_target_node) = previous.successors.clone() else {
            ...
            current = Fragment {
                start_address: cfg.blocks[i].start_address,
                end_address: branch_target_node.start_address(cfg),
                blocks: vec![],
                children: vec![],
                code_entry: child_code,
                root_scope: stack.len() == 1,
                predecessors: vec![],
                successors: Successors::None,
            };
            ...
            let previous = &mut cfg.blocks[i - 1];
            let cur_node_idx = NodeIndex::Fragment(cfg.fragments.len());
            let prev_node_idx = NodeIndex::Block(i - 1);
            previous.successors.set_branch_target(cur_node_idx.clone());
            ...
            current.successors.set_branch_target(branch_target_node);
        }
