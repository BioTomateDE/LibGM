...
pub fn build_chunk_GEN8(data_builder: &mut DataBuilder, ut_data: &UTData) -> Result<(), String> {
    ...
    builder.write_literal_string(&ut_data.general_info.game_file_name.resolve(&ut_data.strings)?)?;
    builder.write_literal_string(&ut_data.general_info.config.resolve(&ut_data.strings)?)?;
    ...
    builder.write_literal_string(&ut_data.general_info.directplay_guid.hyphenated().to_string())?;
    builder.write_literal_string(&ut_data.general_info.game_name.resolve(&ut_data.strings)?)?;
    ...
    builder.write_literal_string(&ut_data.general_info.display_name.resolve(&ut_data.strings)?)?;
