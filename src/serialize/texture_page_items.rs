...
pub fn build_chunk_tpag(builder: &mut DataBuilder, gm_data: &GMData) -> Result<(), String> {
    ...
    let len: usize = gm_data.texture_page_items.textures_by_index.len();
    ...
    for (i, texture_page_item) in gm_data.texture_page_items.textures_by_index.iter().enumerate() {
        builder.resolve_pointer(GMPointer::TexturePageItem(i))?;
