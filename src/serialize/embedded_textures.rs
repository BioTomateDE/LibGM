...
pub fn build_chunk_txtr(data_builder: &mut DataBuilder, gm_data: &GMData, texture_pages: Vec<image::DynamicImage>) -> Result<(), String> {
    ...
    for i in 0..len {
        data_builder.push_pointer_placeholder(&mut builder, GMPointer::TexturePage(i))?;
    }

    for (i, texture_page) in texture_pages.iter().enumerate() {
        data_builder.push_pointer_resolve(&mut builder, GMPointer::TexturePage(i))?;
        ...
        data_builder.push_pointer_placeholder(&mut builder, GMPointer::TexturePageData(i))?;
    }

    for (i, texture_page) in texture_pages.iter().enumerate() {
        data_builder.push_pointer_resolve(&mut builder, GMPointer::TexturePageData(i))?;
