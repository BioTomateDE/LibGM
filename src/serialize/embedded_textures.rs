...
pub fn build_chunk_txtr(data_builder: &mut DataBuilder, gm_data: &GMData, texture_pages: Vec<image::DynamicImage>) -> Result<(), String> {
    ...
    for i in 0..len {
        data_builder.push_pointer_placeholder(&mut builder, GMPointer::texture_page(i))?;
    }

    for (i, texture_page) in texture_pages.iter().enumerate() {
        data_builder.push_pointer_resolve(&mut builder, GMPointer::texture_page(i))?;
        builder.write_u64(0);          // scaled
        if gm_data.general_info.is_version_at_least(2, 0, 6, 0) {
            builder.write_u64(0);      // number of generated mipmap levels
        }
        if gm_data.general_info.is_version_at_least(2022, 3, 0, 0) {
            builder.write_u64(0);      // texture block size  (this placeholder will definitely break)
        }
        if gm_data.general_info.is_version_at_least(2022, 9, 0, 0) {
            builder.write_u64(texture_page.width());
            builder.write_u64(texture_page.height());
            builder.write_usize(i);
        }
        data_builder.push_pointer_placeholder(&mut builder, GMPointer::texture_page_data(i))?;
    }

    for (i, texture_page) in texture_pages.iter().enumerate() {
        data_builder.push_pointer_resolve(&mut builder, GMPointer::texture_page_data(i))?;
