...
pub fn build_chunk_TPAG(data_builder: &mut DataBuilder, ut_data: &UTData, texture_page_items: Vec<UTTexturePageItem>) -> Result<(), String> {
    ...
    for texture_page_item in &texture_page_items {
        data_builder.push_pointer_position(&mut builder, UTRef::Texture(UTTextureRef { index: texture_page_item.texture.index }))?;
    }

    for texture_page_item in &texture_page_items {
        data_builder.push_pointing_to(&mut builder, UTRef::Texture(UTTextureRef { index: texture_page_item.texture.index }))?;
        let texture: &UTTexture = texture_page_item.texture.resolve(&ut_data.textures)?;
...
pub fn generate_texture_pages(ut_textures: UTTextures) -> Result<(Vec<UTTexturePageItem>, Vec<image::DynamicImage>), String> {
    ...
    let texture_count: usize = ut_textures.len();
    let mut textures: Vec<(usize, &UTTexture)> = Vec::with_capacity(texture_count);
    for i in 0..texture_count {
        let texture: UTTextureRef = ut_textures.get_texture_by_index(i).expect("Texture out of bounds while generating texture pages.");
        let texture: &UTTexture = texture.resolve(&ut_textures)?;
    ...
    let mut texture_page_items: Vec<UTTexturePageItem> = Vec::with_capacity(texture_count);
